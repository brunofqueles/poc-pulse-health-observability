...
- `orquestracao/transformar_silver.ipynb` — **7º notebook orquestrador**, transformação Bronze→Silver das 11 tabelas de evento (ADR-013), config-driven via `CONFIGURACAO_TABELAS`. Task de produção criada em setembro/2026, após identificação de uma lacuna real: a função já existia e estava correta desde o ADR-013, mas nunca tinha uma Task chamando ela no `job_diario` (Lição 19)

**Infraestrutura como código:** `databricks.yml` (`mode: production`, `root_path` explícito) + `resources/` (3 Jobs: `job_diario` com **5 Tasks dependentes** — `gerar_dados → ingerir_dados → {promover_seeds, transformar_silver} → construir_gold`, `transformar_silver` adicionada em paralelo a `promover_seeds` em setembro/2026 — e Job Notifications configurado, `job_manutencao` com 1 Task, `job_mensal_fechamento` com 1 Task e cálculo automático de mês anterior — todos com `description`). **Agendamento ativo** (`pause_status: UNPAUSED`) — `job_diario` diariamente às 06:00, `job_manutencao` semanalmente (segunda, 07:00), `job_mensal_fechamento` mensalmente (dia 1, 06:00), America/Sao_Paulo.

**Incidente pós-migração corrigido:** `ModuleNotFoundError` nos 6 orquestradores originais (Repos oferece `sys.path` automático só em `mode: development`) — corrigido com detecção dinâmica de caminho (2 formatos possíveis, com e sem prefixo `/Workspace`, conforme o contexto de execução), validado em 3 camadas: manual, Job isolado, e os 3 Jobs completos via `bundle run` (ADR-004, terceiro adendo; Lição 18). O mesmo padrão precisou ser reaplicado no 7º orquestrador (`transformar_silver.ipynb`), criado depois desse incidente original — dessa vez revelando um detalhe adicional: `dbutils.library.restartPython()` precisa estar em célula própria, sem código na mesma célula, ou a correção do `sys.path` não sobrevive ao restart em execução via Job (Lição 20).
...
**Lacuna de produção identificada e corrigida (setembro/2026):** a transformação Bronze→Silver (ADR-013), com função e configuração corretas desde sempre, nunca teve uma Task de produção no `job_diario` — só era chamada em notebooks de spike e nos dois backfills manuais. A Silver ficou órfã por ~4 semanas (21/08 a 17/09) sem nenhum erro visível, porque `construir_gold` continuava retornando `sucesso` sobre uma Silver estática. Corrigida com a criação do orquestrador `transformar_silver.ipynb` (7º orquestrador), testado isolado com 1 tabela antes de generalizar pras 11, e adicionado como Task em paralelo com `promover_seeds`. Detalhes completos: Lições 19 e 20 (`docs/licoes-aprendidas.md`).

**Alertas e visualização:** 3 canais reais — Job Notifications (execução), `NotificadorTabela` (fallback garantido) e `NotificadorEmail` (Gmail dedicado de portfólio, HTML formatado, senha via Secret Scope — validado em 3 camadas, ADR-007). **AI/BI Dashboard final** (`Pulse - Observabilidade`, ADR-015): 5 painéis — Alertas Recentes, Violações de Cadeia Fria por Tipo, Status de Execução por Pipeline (com `item`, mostrando `distribution` explicitamente), Fechamento Mensal (`gold_fechamento_mensal`), Alertas por Tipo (rosca) — refresh agendado diariamente às 07:00. **Genie Agent** configurado sobre 8 tabelas, validado com 5 perguntas em linguagem natural, incluindo teste deliberado do limite do escopo.
...
**Próximo passo real:** nenhum — roadmap técnico principal completo, incluindo o item de backlog do Agente de Automação e a lacuna de produção da transformação Bronze→Silver, corrigida em setembro/2026. Restam apenas sistemas adicionais (RH, Marketing, Compras) e o treino de apresentação, a critério do autor.

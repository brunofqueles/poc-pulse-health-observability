...
**Incidente pós-migração corrigido:** `ModuleNotFoundError` nos 6 orquestradores (Repos oferece `sys.path` automático só em `mode: development`) — corrigido com detecção dinâmica de caminho (2 formatos possíveis, com e sem prefixo `/Workspace`, conforme o contexto de execução), validado em 3 camadas: manual, Job isolado, e os 3 Jobs completos via `bundle run` (ADR-004, terceiro adendo; Lição 18).

**Próximo passo real:** revisão final do AI/BI Dashboard (incluir `gold_fechamento_mensal`, alertas por tipo com `fechamento_invalido`, e-mails enviados) — último item do roadmap antes do treino de apresentação.
...
**Alertas e visualização:** 3 canais reais — Job Notifications (execução), `NotificadorTabela` (fallback garantido, 520 violações reais processadas) e `NotificadorEmail` (Gmail dedicado de portfólio, senha via Secret Scope, validado em 3 camadas — ver ADR-007, segundo adendo). Primeiro AI/BI Dashboard (`Pulse - Observabilidade`, ADR-015) publicado, com 3 painéis sobre `observability.alertas`, `observability_cadeia_fria` e `pipeline_runs` — as decisões de permissão de publicação são aplicação direta da governança já documentada no ADR-005. **Genie Agent** (`Pulse - Observabilidade`) configurado sobre 8 tabelas (3 Gold + 5 observability), validado com 5 perguntas em linguagem natural — incluindo teste deliberado do limite do escopo (pergunta sobre tabela fora dele, respondida com transparência, sem fabricação) (ADR-015, adendo).
